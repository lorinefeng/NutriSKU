...
export function VisibilityAudit() {
  ...
  const handleRunAudit = async () => {
    ...
    try {
            ...
            setPlatformProgress((prev) =>
              prev.map((item) =>
                item.platform === event.platform
                  ? { ...item, status: "running", completion: 35 }
                  : item
              )
            );
            return;
      ...
      const fallback = buildFallbackAuditResult(form, selectedQuestions, locale);
      setResults(fallback);
  ...
  return (
    <section id="diagnosis" className="relative py-24 md:py-28 overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(200,169,126,0.08),transparent_28%),linear-gradient(180deg,var(--color-void),var(--color-deep),var(--color-surface))]" />
          ...
          <div className="grid lg:grid-cols-[minmax(0,0.8fr)_minmax(640px,1.2fr)] xl:grid-cols-[minmax(0,0.74fr)_minmax(720px,1.26fr)] gap-8 xl:gap-14 items-start">
            <div className="max-w-[560px] xl:pt-3">
              <h2 className="text-3xl md:text-4xl xl:text-[4.35rem] xl:leading-[1.01] font-bold tracking-tight text-[var(--color-text-primary)] mb-4">
                ...
                <span className="gradient-text-gold">
                  {isZh ? "直接把高意图问题跑一遍。" : "Run the intent-rich questions instead."}
                </span>
              </h2>
              <p className="text-base md:text-lg text-[var(--color-text-secondary)] max-w-2xl leading-relaxed mb-7">
                {isZh
                  ? "填写你的品牌或商品信息，我们会生成粗中细三层问题，并尝试直连豆包、Kimi、DeepSeek 做真实检测。平台被拦截时也会明确提示，而不是给你一份看起来很完整但并不可信的假报告。"
                  : "Describe your product or brand and we will generate layered questions, then attempt live checks against Doubao, Kimi, and DeepSeek. If a platform blocks the workflow, we surface that explicitly instead of faking a complete report."}
              </p>
                ...
                {[
                  {
                    ...
                    title: isZh ? "10 个问题" : "10 prompts",
                    desc: isZh ? "粗中细三层自动生成" : "Auto-generated across 3 granularity levels",
                  },
                  {
                    icon: Bot,
                    title: isZh ? "3 个平台" : "3 platforms",
                    desc: isZh ? "豆包 / Kimi / DeepSeek" : "Doubao / Kimi / DeepSeek",
                  },
                  {
                    icon: ShieldAlert,
                    title: isZh ? "真结果优先" : "Real checks first",
                    desc: isZh ? "平台拦截会明确标注" : "Blocked platforms are called out clearly",
                  },
                ].map((item) => (
                  <div key={item.title} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-glass-light)] p-4">
                    <item.icon className="w-4 h-4 text-[#c8a97e] mb-3" />
                    <div className="text-sm font-semibold text-[var(--color-text-primary)] mb-1">{item.title}</div>
                    <div className="text-xs leading-relaxed text-[var(--color-text-muted)]">{item.desc}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="relative w-full max-w-none lg:min-w-[640px] xl:min-w-[720px] lg:ml-auto rounded-[28px] border border-[rgba(200,169,126,0.18)] bg-[linear-gradient(180deg,var(--color-glass),rgba(255,255,255,0.02))] backdrop-blur-xl p-5 md:p-6 xl:p-7 shadow-[0_30px_80px_rgba(0,0,0,0.22)] overflow-hidden">
              ...
              <div className="flex items-center justify-between gap-3 mb-5">
                <div>
                  <div className="text-xs uppercase tracking-[0.22em] text-[#c8a97e] mb-2">Audit Flow</div>
                  <div className="text-lg font-semibold text-[var(--color-text-primary)]">{stageLabel}</div>
                </div>
                <div className="inline-flex items-center gap-2 rounded-full border border-[rgba(200,169,126,0.18)] px-3 py-1.5 text-xs text-[var(--color-text-secondary)]">
              ...
              <AnimatePresence mode="wait">
                {(stage === "intake" || stage === "question_loading") && (
                  ...
                  >
                    ...
                    {stage === "question_loading" ? (
                      <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-5 flex items-center gap-3 text-sm text-[var(--color-text-secondary)]">
                        <LoaderCircle className="w-4 h-4 animate-spin text-[#c8a97e]" />
                        {progressMessage}
                      </div>
                    ) : null}
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      ...
                      <a
                        href="#demo"
                        className="inline-flex items-center justify-center gap-2 rounded-full border border-[rgba(200,169,126,0.28)] px-6 py-3 text-sm text-[#c8a97e] hover:bg-[rgba(200,169,126,0.08)] transition-colors"
                      >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <div className="text-sm font-semibold text-[var(--color-text-primary)]">
                          {isZh ? "推荐先勾选 3-5 个最关键问题" : "Pick 3-5 prompts that matter most"}
                        </div>
                        <div className="text-xs text-[var(--color-text-muted)] mt-1">
                          {isZh
                            ? `已生成 ${questions.length} 个问题，当前选中 ${selectedIds.length} 个。`
                            : `${questions.length} prompts generated, ${selectedIds.length} selected.`}
                        </div>
                      </div>
                      <button
                        type="button"
                        onClick={handleReset}
                        className="inline-flex items-center gap-2 text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors"
                      >
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <div className={cn(
                                ...
                                isChecked
                                  ? "border-[#c8a97e] bg-[#c8a97e] text-[#0c0c10]"
                                  : "border-[var(--color-border-bright)] text-transparent"
                              )}>
                                <CheckCircle2 className="w-3.5 h-3.5" />
                              </div>
                              <div className="flex-1">
                                <div className="flex flex-wrap items-center gap-2 mb-2">
                                  <span className="rounded-full border border-[rgba(200,169,126,0.2)] px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] text-[#c8a97e]">
                                ...
                                <div className="text-sm font-medium leading-relaxed text-[var(--color-text-primary)] mb-2">{question.question}</div>
                                <div className="grid sm:grid-cols-2 gap-2 text-xs leading-relaxed text-[var(--color-text-muted)]">
                                  <div>
                  ...
                  >
                    <div className="rounded-2xl border border-[rgba(200,169,126,0.16)] bg-[rgba(200,169,126,0.06)] p-4">
                      ...
                      <div className="text-xs text-[var(--color-text-muted)] leading-relaxed">
                        {isZh
                          ? "这个阶段会优先尝试真实平台连接；若平台要求登录、触发风控或运行环境不支持浏览器自动化，结果会明确标记为 blocked / error。"
                          : "This step attempts real platform checks first. If a platform requires login, trips risk controls, or the runtime cannot support browser automation, it will be marked as blocked or error explicitly."}
                      </div>
                    </div>
                          ...
                          <div key={platform.platform} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
                            <div className="flex items-center justify-between gap-3 mb-3">
                              <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-full border border-[rgba(200,169,126,0.18)] bg-[rgba(200,169,126,0.08)] flex items-center justify-center text-[#c8a97e]">
                                  {platform.status === "completed" ? <CheckCircle2 className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
                                </div>
                                <div>
                                  <div className="text-sm font-semibold text-[var(--color-text-primary)]">{label}</div>
                            ...
                            <div className="h-2 rounded-full bg-white/[0.04] overflow-hidden mb-2">
                              <div
                                className="h-full rounded-full bg-gradient-to-r from-[#c8a97e] to-[#dfc9a8] transition-all duration-700"
                                style={{ width: `${platform.completion}%` }}
                              />
                            </div>
                            {platform.note ? (
                              <div className="text-xs text-[var(--color-text-muted)] leading-relaxed">{platform.note}</div>
                            ) : null}
                          </div>
                        );
                      })}
                ...
                {stage === "results" && results && (
                  ...
                  >
                    <div className="rounded-[24px] border border-[rgba(200,169,126,0.18)] bg-[linear-gradient(180deg,rgba(200,169,126,0.10),rgba(255,255,255,0.03))] p-5">
                      <div className="inline-flex items-center gap-2 rounded-full border border-[rgba(200,169,126,0.16)] px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-[#c8a97e] mb-3">
                        <CircleAlert className="w-3.5 h-3.5" />
                        {isZh ? "AI 风险扫描结果" : "AI risk scan"}
                      </div>
                      <div className="text-xl font-semibold leading-snug text-[var(--color-text-primary)] mb-2">{results.headline}</div>
                      <div className="text-sm text-[var(--color-text-secondary)] leading-relaxed">{results.subhead}</div>
                    </div>
                    ...
                    <div className="grid lg:grid-cols-[0.95fr_1.05fr] gap-4">
                      <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
                        <div className="text-sm font-semibold text-[var(--color-text-primary)] mb-3">{isZh ? "关键风险" : "Key risks"}</div>
                        <div className="space-y-2.5">
                          {results.keyRisks.map((risk) => (
                            <div key={risk} className="flex gap-3 text-sm text-[var(--color-text-secondary)]">
                              <ShieldAlert className="w-4 h-4 mt-0.5 text-[#c8a97e] shrink-0" />
                              <span>{risk}</span>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
                        <div className="text-sm font-semibold text-[var(--color-text-primary)] mb-3">{isZh ? "建议的 GEO 动作" : "Suggested GEO moves"}</div>
                        <div className="space-y-2.5">
                          {results.suggestions.map((suggestion) => (
                            <div key={suggestion} className="flex gap-3 text-sm text-[var(--color-text-secondary)]">
                              <Sparkles className="w-4 h-4 mt-0.5 text-[#c8a97e] shrink-0" />
                            ...
                            {platform.answers.length === 0 ? (
                              ...
                              <div className="space-y-3">
                                {platform.answers.map((answer) => (
                                  <div key={answer.questionId} className="rounded-2xl border border-white/[0.04] bg-[var(--color-glass-light)] p-4">
                                    <div className="text-sm font-medium text-[var(--color-text-primary)] mb-2">{answer.question}</div>
                                    <div className="grid gap-3 md:grid-cols-[1.1fr_0.9fr]">
                                      <div>
                                        <div className="text-[10px] uppercase tracking-[0.16em] text-[var(--color-text-muted)] mb-2">
                                          {isZh ? "AI 原始回答片段" : "Raw answer snippet"}
                                        </div>
                                      ...
                                      <div>
                                        ...
                                        <div className="text-sm leading-relaxed text-[var(--color-text-secondary)] mb-3">{answer.summary}</div>
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        <div>
                          <div className="text-lg font-semibold text-[var(--color-text-primary)] mb-2">
                            {isZh ? "如果这就是你的真实 AI 可见度，我们建议尽快补位。" : "If this reflects your real AI visibility, we should close the gap fast."}
                          </div>
